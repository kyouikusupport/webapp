...
window.onload = function() {
  ...
  const lapSliderValue = document.getElementById("lap-slider-value");

  let lapCount = 10; // デフォルトのLAP数
  ...
  lapSlider.addEventListener("input", function() {
    lapCount = lapSlider.value;
    lapSliderValue.value = lapCount;
    initializeLapTable();
    updateGraph();
  });

  // 数値入力を直接変更
  lapSliderValue.addEventListener("input", function() {
    const newValue = parseInt(lapSliderValue.value, 10);
    if (newValue >= 8 && newValue <= 40) {
      lapCount = newValue;
      lapSlider.value = lapCount;
      initializeLapTable();
      updateGraph();
    }
  });

  // 初期状態のテーブルを作成、データを保持したまま行数を調整
  function initializeLapTable() {
    lapTableBody.innerHTML = ''; // テーブルをクリア
    for (let i = 1; i <= lapCount; i++) {
      let row = lapTableBody.insertRow();
      row.insertCell(0).textContent = `Lap ${i}`; // Lap番号を表示
      row.insertCell(1).textContent = lapTimes[i - 1] || ''; // 既存の周回タイムを表示
      row.insertCell(2).textContent = lapDifferences[i - 1] || ''; // 既存のラップタイムを表示
    }
  }
  ...
  startButton.addEventListener("click", function() {
    if (timerInterval === null) {
      ...
      timeDisplayElement.style.display = "none"; // スタート時にタイム表示エリアを非表示
    }
  });
  ...
  function updateGraph() {
    chart.data.labels = Array.from({ length: lapCount }, (_, i) => `Lap ${i + 1}`);  // ラベルの数を更新
    chart.data.datasets[0].data = Array(lapCount).fill(null);  // データの配列サイズをlapCountに基づいて更新
    lapDifferences.forEach((diff, index) => {
      if (index < lapCount) {  // lapCountまでのデータのみをグラフに反映
